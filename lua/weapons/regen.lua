AddCSLuaFile()
resource.AddFile("vgui/ttt/sirlag_regeneration.vtf")

ENABLE_SOUND = true		-- Toggles a sound to play each regen tick.
...
TIME_FOR_REGEN = 20		-- How Long (in seconds) it should take to regenerate the heath.
...
hook.Add( "InitPostEntity", "AddRegenToStore", function() 
       local Regen = {
              ...
              loadout = false,
              type = "item_passive",
              material = "vgui/ttt/sirlag_regeneration.vtf",
              name = "Heath Regen",
              desc = "Passively Restores "..HEATH_TO_REGEN.." health over "..TIME_FOR_REGEN.." seconds\n"
      }
	...
	if ply:HasEquipmentItem(EQUIP_REGEN) then
		if(ply.hasregen ~= true) then
			...
			timer.Create( "HeathRegenerationTimer", math.floor(TIME_FOR_REGEN / REGEN_CHUNKS), REGEN_CHUNKS, function()
				...
				if (ENABLE_SOUND)	then surface.PlaySound("buttons/blip1.wav")	end
			end)
		end
	end 
end)
